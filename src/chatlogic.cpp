...
{/*
    ...
    std::cout<< "ipass delete nodes";
    // delete all edges
    for (auto it = std::begin(_edges); it != std::end(_edges); ++it)
    {
        delete *it;
    }
    std::cout<< "ipass delete edges"<<std::endl;
    ////
    //// EOF STUDENT CODE */
}
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            //_edges.emplace_back(std::move(edge)); // Add edge in _edge vector
                        }
    ...
    // TASK 5 
    //std::cout<< "loacl "<<std::endl;
    ChatBot localCahtBot(*_chatBot); 
    
    rootNode->MoveChatbotHere(std::move(localCahtBot));

    //localCahtBot.SetChatLogicHandle(this);
    //localCahtBot.SetRootNode(rootNode);
    //rootNode->MoveChatbotHere(std::move(localCahtBot));
